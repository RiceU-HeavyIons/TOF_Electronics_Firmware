...
int __attribute__((__section__(".script_buffer"))) exeScript(unsigned int board_id)
{
	...
	*/
	...
	for (i=0x10; i<0x15; i++) {
		// Don't do 0x12 and 0x13
		if ((i==0x12) || (i==0x13)) continue;
	...
	j = 0xcfff;	do {--j;} while (j != 0); // idle a little more before checking

	// check for responses, expect 3, 
	// but check more than necessary to account for time of responses to come back
	for (i=1; i<12; i++) {
		// once we received 3 responses, we are finished:
		//if (numRcvd == 8) break;
		if (numRcvd == 3) break;
