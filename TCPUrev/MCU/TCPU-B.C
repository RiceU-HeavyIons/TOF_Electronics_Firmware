...
#else
    #define FIRMWARE_ID_1 0x1
#endif
// WB-11H end

// Define implementation on the TCPU board (I/O ports, etc)
	#define CONFIG_CPU 1		// Make TCPU-B_Board.h define the CPU options
...
// Define our routine includes
	#include "TCPU-B_I2C.h"		// Include prototypes for our I2C routines



    #include "TCPU-B_SPI.h"     // Include for our SPI (EEPROM) macros

    #include "TCPU-B_MCU_PLD.h" // Include for our parallel interface to FPGA

/* DEFINE the HLP_version_3 Packet IDs */
    #include "TCPU-B_CAN_HLP3.h"
...
#if defined (RC15_IO) // RC15 will be I/O (in TCPU-B_Board.h)
...
/* 04-Apr-2007
** Initialize PORTD bits[0,1,4..9] pins [72, 76, 81, 82, 83, 84, 68, 69]
** for control of PLL and EEPROM
*/
// Make D0 an output (pin 72 = PLL_RESET, initialize H)
// Make D1 an output (pin 76 = PLL_LOL, initialize H)
// Make D2 an input  (pin 77 = not used)
// Make D3 an input  (pin 78 = not used)
...
**  97    G.13  Out     0     SEL_TERM2
**  96    G.12  Out     1     PLD_DEVOE
**  14    G.9   Out     0     MCU_SEL_TERM1
**  12    G.8   Out     1     MCU_SEL_LOCAL_OSC
**  11    G.7   Out     1     MCU_EN_LOCAL_OSC
...
//{                   // initialize the CPU oscillator (works with settings in TCPU-B_Board.h)
//    PLLFBD=20;                  /* M= 20*/
//    CLKDIVbits.PLLPOST=0;       /* N1=2 */
//    CLKDIVbits.PLLPRE=0;        /* N2=2 */
