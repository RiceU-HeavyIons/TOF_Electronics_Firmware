...
; this subroutine performs JTAG IR-Scan.  It uses the value in register inst and assumes
; that the TAP controller starts in the Run-Test/Idle state.  it will also return the
; TAP controller to the Run-Test/Idle state when finished.
; It moves the bits onto TDI LSB-first, and assumes a 5-bit instruction word (including parity)
IRScan
	...
	movf	inst,0	; WREG = inst
	andlw	mask0	; WREG = inst AND 0x01
	clr_TDI			; TDI = 0
	bz		$+4
	set_TDI			; TDI = 1 as long as WREG != 0
	str_tck
	...
	movf	inst,0	; WREG = inst
	andlw	mask1	; WREG = inst AND 0x02
	clr_TDI			; TDI = 0
	bz		$+4
	set_TDI			; TDI = 1 as long as WREG != 0
	str_tck
	...
	movf	inst,0	; WREG = inst
	andlw	mask2	; WREG = inst AND 0x04
	clr_TDI			; TDI = 0
	bz		$+4
	set_TDI			; TDI = 1 as long as WREG != 0
	str_tck
	...
	movf	inst,0	; WREG = inst
	andlw	mask3	; WREG = inst AND 0x08
	clr_TDI			; TDI = 0
	bz		$+4
	set_TDI			; TDI = 1 as long as WREG != 0
	str_tck
	...
	movf	inst,0	; WREG = inst
	andlw	mask4	; WREG = inst AND 0x10
	clr_TDI			; TDI = 0
	bz		$+4
	set_TDI			; TDI = 1 as long as WREG != 0
	...
	clr_TMS			; TMS = 0
	clr_TDI			; clear TDI just in case.  Added to match JAM 9/6/2004
	...
	str_tck			; 1
	str_tck			; 2
	str_tck			; 3 These three added to match JAM 9/6/2004
	return
...
;**********************************************************************************
; Fast READ ONLY DRScan
;**********************************************************************************
; this is a streamlined version of DRScan used for reading data ONLY.  This does not 
; put any interesting data on TDI, which saves about 1/2 of the loop time, doubling
; the read speed.  Also, it word-aligns the incoming data so that the 32 bit data
; word is stored in 4 bytes of memory.  It is trivial to implement some sort of check
; of the FIFO_EMPTY bit if desired.
; SEE DRSCAN FOR DETAILS
; if FIFO is empty, WREG = 0xFF.  Otherwise, WREG = 0x00
datagrab
	set_TMS					; TMS = 1
	str_tck					; TAP controller moves to Select-DR-Scan
	clr_TMS					; TMS = 0
	str_tck					; TAP controller moves to Capture-DR
	lfsr	FSR0, dataword1	; INDF0 = &data1
	movlw	0x03			;  WREG = 3
	addwf	FSR0L			; FSR0L = dataword + 3
							; pointer to data LSB
	set_TCK					; TAP controller moves to Shift-DR
	clr_TCK					; TDO = FIFO_empty
	; insert code to break if FIFO_empty here
	grab_TDO_bit			; WREG = TDO (in bit 4)
	btfss	WREG,4			; if TDO = 0, then FIFO is empty
	bra		FIFO_EMPTY 
	set_TCK					; first bit (FIFO empty) gone
...
; bit 0:
	...
	rrncf	WREG	; WREG = TDO (in bit 2)
	rrncf	WREG	; WREG = TDO (in bit 1)
	rrncf	WREG	; WREG = TDO (in bit 0)
	iorwf	TDObyte	; TDObyte: LSB set
	str_tck
; bit 1:
	grab_TDO_bit	; WREG = TDO (in bit 4)
	rrncf	WREG	; WREG = TDO (in bit 3)
...
; bit 7:
	...
	rlncf	WREG	; WREG = TDO (in bit 7)
	iorwf	TDObyte	; TDObyte now holds one word of TDO data
	movff	TDObyte, POSTDEC0	; INDF0 = TDObyte, INDF0--
; We have to set TMS *BEFORE* the next rising edge, so exit the loop here if finished. 
	dcfsnz	dsize		; if dsize = 0, we're done grabbing bits
	bra		got_data	; so go grab the last bit and exit DRScan
; clock in TDI (end of bit 7 of a NON FINAL BYTE):
	set_TCK
	bra		onebyte		; else grab another word of TDO
; Grab the last bit and exit:
got_data
	set_TMS			; TMS = 1
; Next tick leaves Shift-DR
	...
	clr_TMS			; TMS = 0
	str_tck			; TAP to Run-test/Idle
	clrf	WREG	; indicate success
	return
FIFO_EMPTY
	movlw	0xFF	; indicate buffer empty
	return	
